...
void bfs(int node)
{
	...
	while(!q.empty())
	{
		...
		for(int child:adj[curr])
		{
			...
			{
				vis[child]=1;
				newadj[child].pb(curr);
				newadj[curr].pb(child);
				if(child!=1)
				{
					q.push(child);
				}
				else// list to insure that we only add k edges to node 1
				{
					s.insert(curr);
				}
			}
...
void newbfs(int node)
{
	...
	while(!q.empty())
	{
		int curr=q.front();
		q.pop();
		for(int child:newadj[curr])
		{
			if(!vis[child])
			{
				vis[child]=1;
				q.push(child);
				cout<<curr<<tabl<<child<<endl;
			}
		}
	}
}
...
void solve(int ttc)
{
	...
	if(k>=cc and k<=(int)adj[1].size())
	{
		cout<<"YES\n";
		mem0(vis);
	//add necessary nodes into the graph
	// dont dfs from 1 to avoid adding unneccasry nodes
	for(int i=2;i<=n;i++)
	{
		if(!vis[i])bfs(i);
	}	
