...
void dfs(int node,int par)
{
	...
	for(auto child:adj[node])
	{
		...
		if(vis[child])//back edge
		{
			...
			if(in[child]>in[node])//have to check if this not already printed in forward 
			ans.pb(make_pair(node,child));//node should be desencendent of child node or child here should be ancestor of the current node
		}
		else//forward edge
		{
			dfs(child,node);
			...
			{
				hasbridge=true;
				return;//no soltion exist
			}
			else
			{
				low[node]=min(low[node],low[child]);
				ans.pb(make_pair(node,child));
			}
		}
	}
}
