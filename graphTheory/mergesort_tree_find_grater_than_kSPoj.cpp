...
void buildtree(int si,int ss,int se)
{
	...
	int i(0),j(0);
	while(i<st[si*2].size()and j<st[si*2+1].size())
	{
...
int update(int si,int ss,int se,int qs,int qe,int k)
{
if(qe<ss or qs>se)return 0;
if(ss>=qs and se<=qe)
{
	int res;
	res= st[si].end()-upper_bound(all(st[si]),k);
	return res;
}
int mid=ss+(se-ss)/2;
return update(si*2,ss,mid,qs,qe,k)+update(si*2+1,mid+1,se,qs,qe,k);
}
void solve(int ttc)
{
int n;
