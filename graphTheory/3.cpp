...
void update(int si,int ss,int se,int qs,int qe,int u)
{
if(lz[si]!=0)
{
	ll dx=lz[si];
...
int mid=ss+(se-ss)/2;
update(si*2+1,ss,mid,qs,qe,u);
update(si*2+2,mid+1,se,qs,qe,u);
st[si]=st[si*2+1]+st[si*2+2];
}
...
if(pos<=mid)
{
return query(si*2+1,ss,mid,pos);
}
else
{
	return query(si*2+2,mid+1,se,pos);
}
}
void solve(int ttc)
{
