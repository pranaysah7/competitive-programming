...
vi arr(N,0);
int n,m;
bool isvalid(int x,int p)
{
	return (x>=1 and x<=n and x!=p);
}
void bfs(int src,int p)
{
	...
	while(!q.empty())
	{
		...
		q.pop();
			int newcurr=curr+dx;
			if(isvalid(newcurr,curr))
			{
				// vis[newcurr]=1;
				int k=dist[newcurr];
				dist[newcurr]=dist[curr]+1;
				dist[newcurr]=min(k,dist[newcurr]);
				q.push(newcurr);
			}
	}
}
...
for(int i=1;i<=n;i++){scanf("%d",&arr[i]);if(arr[i]!=0)s.insert(i);}
for(auto &a:s)
{
	bfs(a,-1);
}
while(m--)
{
	int x;
	scanf("%d",&x);
	(dist[x]==INT_MAX)?printf("-1 "):printf("%d ",dist[x]);
}
printf("\n");
}
int main()
