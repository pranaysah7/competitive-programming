...
//----------------------------------------------------------------------------------------------------------------
//pr queq;
//push element with zero indeg
//traverse it and delete edge if edge becomes 0 psuh it in the queue
vi adj[100001];
int indeg[100001];
int vis[100001];
vi ans;
...
bool iscylce(int node)
{
	...
	for(int child:adj[node])
	{
		if(!vis[child])
		{
			if(iscylce(child))return 1;
		}
		else
		{
			if(vis[child]==1)return 1;
		}
	}
	vis[node]=2;
	return 0;
...
{
	if(!vis[i])
	(iscylce(i))?cycle=true:cycle=false;
}
if(cycle)
{
cout<<"IMPOSSIBLE";
