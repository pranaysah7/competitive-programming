...
int binarylift(int a,int b,int p)
{
	...
	while(d)
	{
		int i=log2(d);
		b=lca[b][i];
		d-=1<<i;//jumping 2^i jumps
	}
	if(a==b)return a;
	for(;p>=0;p--)
	{
		if(lca[a][p]!=-1 and lca[a][p]!=lca[b][p])//aim is to get  exactly one level beneath lca
		{
			a=lca[a][p];
			b=lca[b][p];
		}
