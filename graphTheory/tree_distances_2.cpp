...
vi adj[N];
	void dfsutil(int node,int par)
	{
		...
		for(int child:adj[node])
		{
			...
			{
				dfsutil(child,node);
				subsize[node]+=subsize[child];//size of sub tree
				subdist[node]+=subsize[child]+subdist[child];//distance of subtree
			}
		}
	}
	void dfs(int node,int par,int n)
	{
		// result of par - subdist of node +subdist of total nodes-current subsize
