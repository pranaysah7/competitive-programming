...
void kahn(int n)
{
	...
	res.pb(curr);
	q.pop();
	for(int child:adj[curr])//egde from curr to child
	{
		indeg[child]--;//delete edge from current node to child to make indegree of the child node tend to 0
		if(indeg[child]==0)//if indeg become 0 push in the queue 
		{
			q.push(child);
		}
	}
